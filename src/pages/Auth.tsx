...
const Auth = () => {
  ...
  const handleAdminLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data: adminUser,
        error: adminError
      } = await supabase.from("admin_roles").select("*").eq("username", adminUsername).eq("password", adminPassword).maybeSingle();
  ...
  const handleRootLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } = await supabase.from("admin_roles").select("*").eq("username", rootUsername).eq("password", rootPassword).eq("role", "super_admin").maybeSingle();
            ...
            <CardContent className="pt-6">
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="login" type="text" placeholder="Введите ваш логин" value={login} onChange={e => setLogin(e.target.value)} />
                </div>
                <div className="space-y-2">
                ...
                <Button type="submit" disabled={tokenLoading} className="w-[48%] mr-2">
            ...
            <CardContent className="pt-6">
              <form onSubmit={handleAdminLoginSubmit} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="adminUsername" type="text" placeholder="Имя администратора" value={adminUsername} onChange={e => setAdminUsername(e.target.value)} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="adminPassword">Пароль</Label>
                  <Input id="adminPassword" type="password" placeholder="Пароль администратора" value={adminPassword} onChange={e => setAdminPassword(e.target.value)} />
                </div>
                {adminError && <div className="text-destructive text-sm">{adminError}</div>}
            ...
            <CardContent className="pt-6">
              <form onSubmit={handleRootLoginSubmit} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="rootUsername" type="text" placeholder="Root имя пользователя" value={rootUsername} onChange={e => setRootUsername(e.target.value)} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="rootPassword">Пароль</Label>
                  <Input id="rootPassword" type="password" placeholder="Root пароль" value={rootPassword} onChange={e => setRootPassword(e.target.value)} />
                </div>
                {rootError && <div className="text-destructive text-sm">{rootError}</div>}
                <Button type="submit" className="w-full flex items-center justify-center" disabled={tokenLoading}>
                  {tokenLoading ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <LogIn className="h-4 w-4 mr-2" />}
